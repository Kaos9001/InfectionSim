...
public class Particle {
    ...
    public boolean inside_spread_zone;
    public double spread_radius;
     ...
     */
    public Particle(double rx, double ry, double vx, double vy, double radius, double mass, Color color, double spread_radius, double recovery_time) {
        this.vx = vx;
        this.vy = vy;
        this.rx = rx;
        this.ry = ry;
    ...
    public void draw(Draw canvas) {
        ...
        if (infected)
        {
            ...
            canvas.circle(rx, ry, spread_radius);
            canvas.setPenColor(Draw.RED);
        }
        else if (recovered)
        {
            canvas.setPenColor(Draw.GRAY);
        }
        canvas.filledCircle(rx, ry, radius);
    }
    ...
    public double[] timeToHit(Particle that) {
        ...
        dts[0] = INFINITY;
        dts[1] = INFINITY;
        if (this == that) return dts;
        double dx  = that.rx - this.rx;
        double dy  = that.ry - this.ry;
        double dvx = that.vx - this.vx;
        double dvy = that.vy - this.vy;
        double dvdr = dx*dvx + dy*dvy;
        if (dvdr > 0) return dts;
        double dvdv = dvx*dvx + dvy*dvy;
        if (dvdv == 0) return dts;
        ...
        if (d >= 0) 
        {
            dts[0] = -(dvdr + Math.sqrt(d)) / dvdv;
        }

        drdr = dx*dx + dy*dy;
        if (that.infected)
        {
            sigma = this.radius + that.spread_radius;
        }
        else
        {
            sigma = this.spread_radius + that.radius;
        }
        
        d = (dvdr*dvdr) - dvdv * (drdr - sigma*sigma);
        if (d >= 0) dts[1] = -(dvdr + Math.sqrt(d)) / dvdv;
